...
get_instance_tags () {
    ...
    echo $(aws ec2 describe-tags --region ap-southeast-2 --filters "Name=resource-id,Values=$instance_id")
}

get_ami_tags () {
    ami_id=$(/usr/bin/curl --silent http://169.254.169.254/latest/meta-data/ami-id)
    echo $(aws ec2 describe-tags --region ap-southeast-2 --filters "Name=resource-id,Values=$ami_id")
}
...
tags_to_env () {
    ...
    for key in $(echo $tags | jq -r ".[][].Key"); do
        ...
        key=$(echo $key | tr '-' '_' | tr ':' '_' | tr '[:lower:]' '[:upper:]')
        export $key="$value"
